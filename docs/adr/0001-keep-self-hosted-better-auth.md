...
- The application requires `BETTER_AUTH_SECRET` in every environment and refuses to start without it. A shared development fallback was removed: a deploy that forgot the secret would otherwise have signed its cookies with a value published in this repository. (Superseded by ADR-0023: the Worker generates the secret on first use and stores it in D1.)
